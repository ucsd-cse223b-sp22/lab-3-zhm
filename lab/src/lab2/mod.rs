...
//! - `shutdown: Option<Receiver<bool>>`: A shutdown signal channel. If a
//!   shutdown channel is provided, When any message is received on this
//!   channel, shut down the keeper. If no channel is given, then the keeper
...
//!
//! ```rust
//! pub async fn new_front(bin_storage: Box<dyn BinStorage>) -> TribResult<Box<dyn Server>>
//! ```
//!
